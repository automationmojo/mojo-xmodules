...
from mojo.clients.osx.osxclient import OsxClient
...
def format_osx_client_configuration_error(message, osxdev_config):
    """
        Takes an error message and an OSX client configuration info dictionary and
...
class OsxCoordinator(CoordinatorBase):
    """
    ...
    def create_landscape_device(self, landscape: "Landscape", device_info: Dict[str, Any]) -> Tuple[FriendlyIdentifier, OsxClient]:
        """
        ...
        device = OsxClient(landscape, self, fid, dev_type, device_info)
        
        coord_ref = weakref.ref(self)
        device_ref = weakref.ref(device)

        ssh_agent = SshAgent(host, device.osx_credential)
    ...
    def extend_devices(self, osxdevices: List[LandscapeDevice], upnp_coord: Optional[CoordinatorBase]=None):
        ...
        osx_devices_available = []
        osx_devices_unavailable = []

        for osxdev in osxdevices:
            osxdev_config = osxdev.device_config
            osx_credential_list = osxdev.osx_credentials
            if len(osx_credential_list) == 0:
                errmsg = format_osx_client_configuration_error(
            ...
            priv_cred = osx_cred_by_role["priv"]
            ...
            if host is not None:
                ...
                try:
                    ...
                    if status == 0 and stdout.strip() == "Hello":
                        osx_devices_available.append(osxdev_config)
                    else:
                        osx_devices_unavailable.append(osxdev_config)
                except:
                    osx_devices_unavailable.append(osxdev_config)
                ...
                # OsxPoolCoordinator is repsonsible for activating it
        ...
        self._available_devices = osx_devices_available
        self._unavailable_devices = osx_devices_unavailable

        return osx_config_errors, osx_devices_available, osx_devices_unavailable
