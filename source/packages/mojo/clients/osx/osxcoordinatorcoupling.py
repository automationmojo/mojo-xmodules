"""
.. module:: osxcoordinatorcoupling
    :platform: Darwin, Linux, Unix, Windows
    :synopsis: Contains a OsxCoordinatorCoupling object to use for working with the OSX clients.
...
from mojo.clients.osx.osxcoordinator import OsxCoordinator
...
def is_osx_client_config(device_info):
    is_osx_client = False
    ...
    return is_osx_client

class OsxCoordinatorCoupling(CoordinatorCoupling):
    """
        The OsxCoordinatorCoupling handle the requirement registration for the OSX coordinator.
    """
    ...
    def attach_to_environment(cls, landscape: "Landscape"):
        ...
        if len(device_list) > 0:
            osx_client_list = [dev for dev in filter(is_osx_client_config, device_list)]

            if len(osx_client_list) > 0:
    ...
    def create_coordinator(cls, landscape: "Landscape") -> object:
        ...
        """
        cls.coordinator = OsxCoordinator(landscape)
    ...
    def establish_connectivity(cls, allow_missing_devices: bool=False) -> Tuple[List[str], dict]:
        ...
        if len(device_list) > 0:
            ...
            osx_config_errors, matching_device_results, missing_device_results = cls.coordinator.attach_to_devices(
                osx_client_list)
        ...
        return (osx_config_errors, osx_scan_results)
